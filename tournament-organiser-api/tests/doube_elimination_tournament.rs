...
async fn get_tournament(db: PgPool) {
    ...
    let matches: Vec<Match> = bracket.bracket.get_matches().to_owned();
    ...
    let state = BracketState {
        ...
        results: vec![
            PlayerMatchResultReport {
                player1_id: *p2.clone().get_id(),
                player2_id: *p3.clone().get_id(),
                score_p1: 2,
                score_p2: 0,
            },
            PlayerMatchResultReport {
                player1_id: *p1.clone().get_id(),
                player2_id: *p2.clone().get_id(),
                score_p1: 2,
                score_p2: 0,
            },
            PlayerMatchResultReport {
