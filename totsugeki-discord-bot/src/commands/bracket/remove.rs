//! TO can remove player from bracket if it's known he cannot show up (before
//! bracket starts)
...
use totsugeki_api_request::remove::post;
...
async fn remove(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    span.in_scope(|| async {
        ...
        msg.reply(
            ctx,
            format!("Removed {player_id} from bracket {bracket_id}"),
        )
        .await?;
