...
mod tests {
    ...
    fn run_5_man_bracket() {
        ...
        let auto = false;
        let s = Step::new(None, seeding.clone(), auto).expect("seeding");

        let (matches, match_id, _) = s
            .tournament_organiser_reports_result(p[5].get_id(), (2, 0), p[4].get_id())
            .expect("winner 4vs5");
        let s = Step::new(Some(matches), seeding.clone(), auto).expect("s");
        ...
        let s = Step::new(Some(matches), seeding, auto).expect("s");
