...
mod tests {
    ...
    fn disqualifying_player_sets_looser_of_their_current_match() {
        ...
        let condition = s.matches.iter().any(|m| {
    ...
    fn assert_player_drops_to_losers(s: &Step, n: usize, p: &[Player]) {
        let (winners, losers, _, _) =
            partition_double_elimination_matches(&s.matches, s.bad_seeding.len())
                .expect("partition");
        assert!(
    ...
    fn disqualifying_everyone_is_impossible_because_the_last_player_remaining_wins_grand_finals_automatically(
        ...
        let (_w_bracket, l_bracket, _, _) =
            partition_double_elimination_matches(&s.matches, s.bad_seeding.len()).expect("p");
        let m = &l_bracket
            .iter()
    ...
    fn disqualifying_most_in_double_elimination_tournament_and_lowest_expected_seed_in_winners_final(
        ...
        let seeding = bad_seeding
            ...
            .collect::<Vec<_>>();
        let auto = true;
        let s = Step::new(None, bad_seeding.clone(), seeding, auto).expect("step");

        let (matches, _) = s.disqualify_participant(p[8]).expect("dq 8");
        let s = Step::new(
            Some(matches),
            bad_seeding.clone(),
            bad_seeding
        ...
        .expect("step");
        let (matches, _) = s.disqualify_participant(players[7].get_id()).expect("dq 7");
        let s = Step::new(Some(matches), bad_seeding.clone(), s.seeding, auto).expect("step");
        let (_w_bracket, l_bracket, _, _) =
            partition_double_elimination_matches(&s.matches, bad_seeding.len()).expect("partition");
        assert!(
        ...
        let (matches, _) = s.disqualify_participant(players[6].get_id()).expect("dq 6");
        let s = Step::new(Some(matches), bad_seeding.clone(), s.seeding, auto).expect("step");
        let (_, l_bracket, _, _) =
            partition_double_elimination_matches(&s.matches, bad_seeding.len()).expect("partition");
        assert_x_wins_against_y(&players[6], &players[7], &l_bracket);

        let (matches, _) = s.disqualify_participant(players[5].get_id()).expect("dq 5");
        let s = Step::new(Some(matches), bad_seeding.clone(), s.seeding, auto).expect("step");
        let (matches, _) = s.disqualify_participant(players[4].get_id()).expect("dq 4");
        let s = Step::new(Some(matches), bad_seeding.clone(), s.seeding, auto).expect("step");
        let (matches, _) = s.disqualify_participant(players[3].get_id()).expect("dq 3");
        let s = Step::new(Some(matches), bad_seeding.clone(), s.seeding, auto).expect("step");
        let (matches, new_matches) = s.disqualify_participant(players[2].get_id()).expect("dq 2");
        let s = Step::new(Some(matches), bad_seeding, s.seeding, auto).expect("step");

        assert_eq!(new_matches.len(), 0);
        assert!(s.matches[s.matches.len() - 2].contains(players[1].get_id()),);
        assert!(s.matches[s.matches.len() - 2].contains(players[2].get_id()),);
        assert_eq!(
        ...
        assert_eq!(
            ...
            Opponent::Player(players[1].clone()),
    ...
    fn disqualify_from_winner() {
        ...
        let s = Step::new(Some(bracket), seeding, s.seeding, auto).expect("step");
    ...
    fn disqualify_in_double_elimination_bracket_from_loser() {
        ...
        let (bracket, _, new_matches) = s
            .tournament_organiser_reports_result(p[2].get_id(), (2, 0), p[3].get_id())
            .expect("to report");
        let s = Step::new(Some(bracket), seeding.clone(), s.seeding, auto).expect("step");
        assert_eq!(new_matches.len(), 1, "expected 1 new match");
        let (bracket, _, new_matches) = s
            .tournament_organiser_reports_result(p[1].get_id(), (2, 0), p[2].get_id())
            .expect("to report");
